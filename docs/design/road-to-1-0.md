...
| 6 | wrecked units on the client: applied count, tracker count, corpse not orderable/targetable | M17 stage 2 acceptance | L3's counters + `pbj.drive-state` | applied=0 with host-wrecks>0 = apply path dead; the two counts print side by side |
| 7 | 🧑 client corpse stays collapsed through planning (eyes) | M17 stage 2 | the user watches instance 3 | n/a — human reading |
...
| 5 | **PR-5: M17 stage 2 — WINDOW W2** | L3 | wire **v10** + ModVersion **0.24.0** (UnitSnapshot + codec) | 🧑 user authorizes the build (plan §9; 3rd refutation already paid); `make record-wire-surface` **and `record-split-grouping`** same commit; `Closed`/`Faulted` predicate; `pbj.force-end` |
