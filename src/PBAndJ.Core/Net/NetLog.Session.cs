...
{
    // Standing a session up and getting a peer into it: listening, connecting,
    // the handshake and what it settles, and closing down.
    //
    // Status and NoSession are here rather than under the outbound queue where the
    // original filed them. They are what `pbj.status` prints about the session, not
    // about the queue -- they are the only two members in that section that are not
    // about sending, and the test split reached the same conclusion independently.
    //
